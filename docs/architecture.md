...
- `src/extension/content-script.ts` is the content-script entry point; it coordinates the observed new-composer and reply-composer lifecycles.
- `src/extension/korus-new-composer.ts` is the KORUS integration adapter for the observed composer page states, visible body selector, and text-safe insertion operation.
...
- The composer adapter reads only the configured phrase and mutates only the validated visible editor on the exact observed pages; unsupported page states fail closed.
